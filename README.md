...
If Vercel shows `FUNCTION_INVOCATION_FAILED`, open the Vercel project logs and check the API route that crashed. The deployed app uses:

- `/api/status`
- `/api/launch-plan`

Both routes import shared runtime logic from `lib/launchops.js`, so the serverless functions do not depend on the local `local-server.js` process.
